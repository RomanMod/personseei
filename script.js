// Diagnostic log to confirm script loading
...
try {
    ...
    const requiredElements = {
        ...
        'next-photo': nextPhotoBtn,
        'check-answer': checkAnswerBtn,
        'person-image': personImage,
        'progress': progress,
        'result': result,
        'person-info': personInfo,
        'wiki-link': wikiLink,
        'correct-answers': correctAnswersSpan,
        'total-questions': totalQuestionsSpan
    };
    for (const [id, element] of Object.entries(requiredElements)) {
        if (!element) {
            console.error(`Element #${id} not found in DOM`);
            if (progress) {
                progress.textContent = `Помилка: елемент #${id} не знайдено`;
                progress.classList.add('init-error');
            }
    ...
    // Set player name
    playerName.textContent = tg.initDataUnsafe.user ? 
    ...
    const translations = {
        uk: {
            ...
            initError: 'Помилка ініціалізації програми'
        },
        ru: {
            alive: 'Жив',
            dead: 'Мертв',
        ...
        en: {
            ...
            noPerson: 'Person not loaded, try again',
    ...
    async function loadNewPerson(useMock = false) {
        ...
        progress.classList.remove('error', 'mock', 'init-error');
        if (imageError) imageError.style.display = 'none';
        result.style.display = 'none';
        personImage.style.display = difficulty === 'easier' ? 'block' : 'none';
        personImage.src = '';
        ...
        if (useMock || retryCount > maxRetries) {
            ...
            progress.classList.remove('loading', 'error', 'init-error');
            if (imageError) imageError.style.display = 'none';
            personImage.src = currentPerson.image;
            checkAnswerBtn.disabled = false;
        ...
        const loadPromise = (async () => {
            try {
                ...
                progress.classList.remove('loading', 'mock', 'init-error');
                if (imageError) {
 ...
 vergüenza, 'init-error');
    ...
    const progress = document.getElementById('progress');
    if (progress) {
        progress.textContent = 'Помилка ініціалізації програми';
        progress.classList.add('init-error');
    }
}
